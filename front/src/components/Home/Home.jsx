...
const Home = observer(({user, cart}) => {
    ...
    const currency = [
      {id: 1, name: "BYN", rate: exchangeRates.BYN}, 
      {id: 2, name: "RUB", rate: exchangeRates.RUB}, 
      {id: 3, name: "USD", rate: exchangeRates.USD}, 
      {id: 4, name: "EUR", rate: exchangeRates.EUR}]
    ...
    const renderData = () => {
      ...
      const renderPageNumbers = () => {
        ...
        for (let i = 1; i <= Math.ceil(filteredProductsArr.length / itemsPerPage); i++) {
          pageNumbers.push(
            <span key={i} onClick={() => handleClick(i)} style={{ 
              cursor: 'pointer', 
              margin: '0 5px', 
              fontWeight: 
      ...
      const setNameFilter = async(event) => {
      ...
      const setCategoryFilter = async(event) => {
    ...
    return (
        <div className="home">
            <div className="filter">
                ...
                <input type="text" className="search" placeholder="Search..." onChange={async(event) => {setNameFilter(event)}}/>
                <select className="select-category" value={value} onChange = {async (event) => {setCategoryFilter(event)}}>
